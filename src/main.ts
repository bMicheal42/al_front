...
bootstrap.getConfig().then(config => {
  ...
  Vue.prototype.$shared.set = (key: string, value: any) => {
    Vue.prototype.$shared[key] = value
  }
  ...
  Vue.use(GoogleAnalytics, {
    // trackingId: config.tracking_id, // disabled analytics for cleaner console
    router
  })
  sync(store, router)

  // FIXME disable for prod in future time
